...
module.exports = runWith = Bluebird.coroutine ({cwd, env, containingDir, inputs: [dirName, templateName], resolveValues, log}) ->
  ...
  cdExists = yield fsExists containingDir
  ...
  configFilePath = yield detectSeries configDirs, fsExists
  ...
  templateDir = yield detectSeries templateDirs, fsExists
  throw new Error "Cannot find template in #{templateDirs.join ", or "}." unless templateDir
  log "Using template #{templateDir}."
  creatingProject = yield createProject targetDir, templateDir, getValues
